...
Safety and compliance primitives for AI agents operating on Base mainnet — covering emergency stop controls, identity verification, tamper-proof audit trails, bounties, IP licensing, milestone payments, recurring subscriptions, debt management, referral networks, and collective treasuries.
...
Three steps to secure your agent on-chain:

```
1. Register kill switch → AgentKillSwitch.registerAgent{value: 0.01 ether}(agent, 1000_000_000, 100, 86400)
2. Submit KYA          → AgentKYA.submitKYA(ownerName, jurisdiction, purpose, maxSpend, supervised, docHash)
3. Log agent action    → AgentAuditLog.logAction{value: 0.0001 ether}(agent, keccak256("TRANSFER"), dataHash, amount)
```
