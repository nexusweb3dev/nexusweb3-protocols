...
NexusWeb3 Utility gives your agent operational superpowers: on-chain task scheduling, price feeds, collective voting, persistent storage, agent-to-agent messaging, staking with revenue share, access control lists, USDC auctions, revenue splitting, and ecosystem analytics. All on Base mainnet. All audited. 10 protocols.
...
Three steps to start using the utility layer:

```
1. Schedule task   → AgentScheduler.scheduleTask(taskData, executeAfter, repeatInterval, maxExecutions)
2. Get price feed  → AgentOracle.getLatestValue(feedId)
3. Split payment   → AgentSplit.createSplit(recipients, shares, description)
```
